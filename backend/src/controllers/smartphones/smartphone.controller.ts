...
class smartPhoneController {
  ...
  async getSmartphonesByUserId(
    ...
    try {
      ...
      const smartphones =
        await smartphoneService.getSmartphonesByUserId(userId);
  ...
  async addSmartphone(req: Request, res: Response, next: NextFunction) {
    try {
      const { name, description, specifications, imageUrl, brandId, price } =
        req.body;
      ...
      const smartphone = await smartphoneService.addSmartphone(
        userData.id,
        name,
        description,
        specifications,
      ...
      return res
        .status(200)
        .json({ message: `создан смартфон с id: ${smartphone.id}` });
    } catch (e) {
      next(e);
    }
  }

  async getSmartphonesByUsername(req: Request, res: Response) {
    const username = req.params.username as string;
    const smartphones =
      await smartphoneService.getSmartPhonesByUsername(username);
    return res.status(200).json(smartphones);
  }
}
