...
export function CreateSmartphoneForm() {
  ...
  const onSubmit: SubmitHandler<CreateSmartphoneData> = async (data) => {
    ...
    if (response.status === 200) {
      ...
      formMethods.setError("SmartphoneError", {
  ...
  const BrandError = formMethods.formState.errors.brandId;
  const PriceError = formMethods.formState.errors.price;
  ...
  return (
    <div className="flex justify-center items-center w-full min-h-[90vh]">
      <form onSubmit={formMethods.handleSubmit(onSubmit)} className="w-[30%]">
        <h1 className="text-4xl mb-2 text-white font-extrabold">
          Публикация смартфона
        </h1>
        <p className="text-white/40 mb-8">
          Заполните всю информацию о смартфоне
        </p>
        <CustomInput
          type="text"
        ...
        <div className="grid grid-cols-2 gap-4 mb-4">
          <div>
            ...
            <BrandSection
              ...
              brands={brands}
            />
            {BrandError && <FormError error={BrandError} />}
          </div>
          <div>
            <label htmlFor="" className="ml-1">
              Цена
            ...
            <PriceInput
              ...
              maxPrice={99999999}
            />
            {PriceError && <FormError error={PriceError} />}
          </div>
        </div>
        <label htmlFor="" className="ml-1">
          Описание
        ...
        <textarea
          ...
          className="w-full h-32 p-3 bg-white/5 
        ...
        <textarea
          ...
          className="w-full h-32 p-3 mb-4 bg-white/5 
                    border border-white/20 rounded-2xl 
                    focus:outline-none focus:border-white/60
                    focus:shadow-xl focus:shadow-white/10
                    transition-colors duration-200"
        ...
        />
        {SmartphoneError && <FormError error={SmartphoneError} />}
