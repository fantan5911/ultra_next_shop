...
export function CartItemCard({ cartItem }: Props) {
  ...
  const debouncedSearchTerm = useDebounce(count.toString(), 500);
  ...
  const updateCartItemCount = async () => {
    const newCount = parseInt(debouncedSearchTerm);
    console.log("обновили количество", newCount);
    ...
    await cart_itemService.updateCartItemCount(cartItem.id, newCount);
  };

  useEffect(() => {
    updateCartItemCount();
  }, [debouncedSearchTerm, cartItem.id]);

  return (
    <div
      className="flex justify-between w-full lg:h-[15%] bg-white/5
    ...
    >
      <div className="flex items-center gap-4 hover:text-underline transition-colors duration-300">
        ...
        <div className="flex flex-col gap-2">
          <Link
            href={PAGES.SMARTPHONE(cartItem.smartphoneId)}
          ...
          <div className="flex items-center gap-4">
            <button
              className="px-3.5 py-1 bg-white/10 rounded hover:bg-white/20 cursor-pointer
